...
def show_prediction(img,model,class_names,show = False,threshold = 0.6): 
    ...
    if pred_prob.item() > threshold:
        brand = class_names[predicted]
        print("Predicted Class: "+ brand+"|| Confidence: "+str(pred_prob[0].tolist()))
        if show:
            ...
            ax.set_title(f"Predicted Brands: {brand}")
            plt.show()
            return brand
