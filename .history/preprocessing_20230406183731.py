...
def create_label_classification(annotations):
    ...
    with open("classification label.txt",'w') as f:
        for group in grouped_annotations:
            ...
            brand_id = get_id_by_brand(group[1].iloc[0, 1], name_map)
            f.write(f"{filename} {brand_id} \n")    
...
create_label_detect(annonations)
create_label_classification(annonations)