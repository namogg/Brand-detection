...
results = model.predict(source = "E:/Brand detect/Brand-detection/test/videos/apple.mp4" , conf = 0.2, show=True) 
#cv2.waitKey()
...
for result in results: 
    ...
    if bbox.numel():
        ...
        brand = show_prediction(img_tensor,model_classification,class_names)
        if brand not in brand_list:
            brand_list.append(brand)

print("Brand detected: "+len(brand_list))
for i in len(brand_list)
