...
cv2.waitKey()
print()
boxes = result.boxes.xyxy.to(device).numpy().astype(int)
confidences = result.boxes.conf.to(device).numpy().astype(float)
labels = result.boxes.cls.to(device).numpy().astype(int) 
